...
EXEC sp_CalculateEmployeeBonus @BonusPercentage = 10;
...
BEGIN
    UPDATE Employees
    SET Salary = Salary + (Salary * @IncreasePercentage / 100.0)
...
EXEC sp_UpdateSalaryByDepartment @DepartmentName = 'Sales', @IncreasePercentage = 10;
...
SELECT 
    id,
    CASE
        WHEN p_id IS NULL THEN 'Root' -- ota tuguni yo'q, ildiz tugun
        WHEN id NOT IN (SELECT DISTINCT p_id FROM Tree WHERE p_id IS NOT NULL) THEN 'Leaf' -- farzandlari yo'q
        ELSE 'Inner' -- ildiz ham emas, barg ham emas
    END AS type
FROM Tree;
...
CREATE TABLE Confirmations2 (
    user_id INT,
    time_stamp DATETIME,
    action ENUM('confirmed', 'timeout')
);

INSERT INTO Confirmations2 (user_id, time_stamp, action) VALUES 
