...
JOIN
	Employee manager ON employe.ManagerID = manager.Id
WHERE employe.Salary > manager.Salary;
...
JOIN 
	Weather weather2 ON weather1.RecordDate = DATEADD(DAY, 1, weather2.RecordDate)
...
FROM GetIntegers
WHERE ISNUMERIC(SUBSTRING(Vals, 1, PATINDEX('%[^0-9]%', Vals + 'a') - 1)) = 1;
...
SELECT 
    Vals,
    CONCAT(SUBSTRING(Vals, 3, 1), SUBSTRING(Vals, 1, 1), SUBSTRING(Vals, 5, LEN(Vals)-2)) AS SwappedVals
FROM MultipleVals;
...
WITH WeeklySales AS (
    SELECT 
        ...
        DATEPART(WEEK, date) AS sales_week,
        area,
        SUM(SalesRemote) AS weekly_area_sales
    FROM WeekPercentagePuzzle
    GROUP BY DATEPART(YEAR, date), DATEPART(WEEK, date), area
),
DailySales AS (
    SELECT 
        DATEPART(YEAR, date) AS sales_year,
        ...
        SUM(SalesRemote) AS daily_area_sales
    FROM WeekPercentagePuzzle
    GROUP BY DATEPART(YEAR, date), DATEPART(WEEK, date), area, DATENAME(WEEKDAY, date)
)
SELECT 
    d.sales_year,
    d.sales_week,
    ...
    d.daily_area_sales,
    w.weekly_area_sales,
    CAST(100.0 * d.daily_area_sales / w.weekly_area_sales AS DECIMAL(5,2)) AS percent_of_week
FROM DailySales d
JOIN WeeklySales w
  ON d.sales_year = w.sales_year 
 AND d.sales_week = w.sales_week 
 AND d.area = w.area
ORDER BY d.sales_year, d.sales_week, d.area, d.day_name;
