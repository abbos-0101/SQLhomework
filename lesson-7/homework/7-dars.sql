...
CREATE TABLE Customers ( 
		...
		Phone VARCHAR(50), Address VARCHAR(255), 
		City VARCHAR(100), State VARCHAR(100), 
		PostalCode VARCHAR(20), Country VARCHAR(100)
		)
...
SELECT SUM(SaleAmount) AS SumId
FROM Sales
WHERE ProductID = 7
...
SELECT COUNT(EmployeeID) AS HRCount
FROM Employees
...
SELECT Category, AVG(SaleAmount) AS AverageSales
FROM Sales
...
SELECT CustomerID,
    MAX(SaleAmount) AS MaxOrderValue,
    MIN(SaleAmount) AS MinOrderValue
FROM Sales
GROUP BY CustomerID
...
SELECT 
    ...
    COUNT(DISTINCT ProductID) AS DistinctSold
FROM Sales
