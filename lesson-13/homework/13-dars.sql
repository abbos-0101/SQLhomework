...
select concat(EMPLOYEE_ID, '-', FIRST_NAME, space(1), LAST_NAME) as result
From Employees
...
--    hire_date,
--    ROUND(
--        (YEAR('2025-05-01') - YEAR(hire_date)) - 
--        CASE 
--            WHEN MONTH('2025-05-01') < MONTH(hire_date) 
--                OR (MONTH('2025-05-01') = MONTH(hire_date) AND DAY('2025-05-01') < DAY(hire_date)) 
--            THEN 1 
--            ELSE 0 
--        END, 2) AS years_of_service
--FROM employees
--WHERE (YEAR('2025-05-01') - YEAR(hire_date)) BETWEEN 10 AND 14;
...
--) avg_departament
--ON employee.department_id = avg_departament.department_id
--WHERE employee.salary > avg_departament.avg_salary;
---------------------------------------------------------------------
----            MEDIUM TASK          ------------
...
--    IF UNICODE(@ch) BETWEEN UNICODE('A') AND UNICODE('Z')
--        SET @Upper += @ch;
--    ELSE IF UNICODE(@ch) BETWEEN UNICODE('a') AND UNICODE('z')
--        SET @Lower += @ch;
--    ELSE IF UNICODE(@ch) BETWEEN UNICODE('0') AND UNICODE('9')
--        SET @Digits += @ch;
--    ELSE
--        SET @Others += @ch;
...
SELECT 
  STRING_AGG(String, ' ') AS FullQuery
FROM DMLTable;
...
SELECT 
    JobDescription,
    MAX(MissionCount) AS MostCount,
    MIN(MissionCount) AS LeastCount
FROM Personal
...
SELECT 
    ...
    s.FullName,
    s.Grade,
    (SELECT SUM(Grade) FROM Students WHERE StudentID <= s.StudentID) AS RunningTotal
FROM Students s
